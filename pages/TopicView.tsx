...
import { ChevronRight, LogOut, Video, Loader2, StopCircle, Headphones, Sparkles, ArrowLeft, ArrowRight, CheckCircle, RefreshCcw, Languages } from 'lucide-react';
...
const TopicView: React.FC<Props> = ({
  activeChapter, activeTopic, currentAiVersion, aiOverrides, setAiOverrides, quizScores,
  onNavigate, handlePrevTopic, handleNextTopic, handleTopicQuizPass,
  onRegenerate, onGenerateVideo, onToggleAudio, isRegenerating, isGeneratingVideo, isGeneratingAudio, isPlayingAudio
}) => {
  ...
  const handleRegenSubmit = () => {
      ...
      if (!finalPrompt) {
          finalPrompt = "Explain this topic thoroughly.";
      }
  ...
  const currentTopicIndex = activeChapter.topics.findIndex((t: any) => t.id === activeTopic.id);
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 md:px-6 pb-40 relative">
        ...
        <nav className="flex items-center text-sm font-bold mb-8 bg-white dark:bg-black border-2 border-black dark:border-zinc-700 px-4 py-2 rounded-lg shadow-[4px_4px_0px_0px_#000] w-fit">
            <span className="text-gray-500 hover:text-black dark:hover:text-white cursor-pointer transition-colors" onClick={() => onNavigate('chapter', undefined, activeChapter.id)}>{activeChapter.title}</span>
        ...
        <div className="bg-white dark:bg-black rounded-xl border-4 border-black dark:border-zinc-700 p-8 md:p-12 shadow-[8px_8px_0px_0px_#000] dark:shadow-[8px_8px_0px_0px_#333]">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-10 border-b-4 border-black dark:border-zinc-800 pb-6">
                <h1 className="text-3xl md:text-5xl font-black text-black dark:text-white uppercase tracking-tighter leading-none">{activeTopic.title}</h1>
                <div className="flex gap-3 shrink-0">
                     ...
                     {currentAiVersion && (
                        <button onClick={() => { const n = {...aiOverrides}; delete n[activeTopic.id]; setAiOverrides(n); }} className="p-2 bg-white dark:bg-zinc-900 border-2 border-black dark:border-zinc-500 text-black dark:text-white rounded-lg hover:bg-red-50 hover:text-red-600 shadow-[3px_3px_0px_0px_#000]"><LogOut size={18}/></button>
                     )}
                </div>
            </div>
            ...
            {showPromptInput && (
                <div className="mb-10 bg-acid dark:bg-zinc-900 p-6 rounded-xl border-4 border-black dark:border-white animate-in fade-in slide-in-from-top-4 shadow-[6px_6px_0px_0px_#000]">
                    <label className="block text-xs font-black uppercase text-black dark:text-white mb-2 tracking-widest">CUSTOM INSTRUCTION</label>
                    <textarea 
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-start text-xs">
                                <span className="font-black uppercase tracking-wider">{useHinglish ? 'HINGLISH: ON' : 'HINGLISH: OFF'}</span>
                                <span className="text-[10px] font-medium opacity-80">{useHinglish ? 'Explains in Roman Hindi' : 'Explains in English'}</span>
                            </div>
                            <div className={`w-4 h-4 rounded-full border border-current ml-2 flex items-center justify-center`}>
                                {useHinglish && <div className="w-2 h-2 rounded-full bg-current"></div>}
                            </div>
                        </button>

                        <div className="flex justify-end gap-3 w-full sm:w-auto">
                            <button onClick={() => setShowPromptInput(false)} className="px-5 py-2 font-bold text-black dark:text-white hover:underline">Cancel</button>
                            <button onClick={handleRegenSubmit} className="px-6 py-2 bg-black text-white dark:bg-white dark:text-black font-bold rounded-lg hover:scale-105 transition-transform shadow-[3px_3px_0px_0px_rgba(255,255,255,0.5)]">Generate</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isRegenerating ? (
                ...
                <div className="text-lg leading-relaxed">
                    <MarkdownRenderer content={currentAiVersion ? currentAiVersion.content : activeTopic.content} />
                </div>
            )}
        </div>
        ...
        <TopicQuiz 
            ...
            topicContent={currentAiVersion ? currentAiVersion.content : activeTopic.content}
        ...
        <div className="sticky bottom-6 z-30 mt-12">
             <div className="bg-white dark:bg-zinc-900 p-3 rounded-2xl shadow-[0px_10px_40px_-10px_rgba(0,0,0,0.2)] border-4 border-black dark:border-zinc-600 flex items-center justify-between gap-4 max-w-2xl mx-auto">
                 ...
                 >
                    <ArrowLeft size={20} /> <span className="hidden sm:inline uppercase tracking-wider">Previous</span>
                 </button>
        ...
        {!isRegenerating && (
             <div className="fixed bottom-32 right-8 flex flex-col gap-4 z-40">
                 ...
                 >
                     {isGeneratingVideo ? <Loader2 size={24} className="animate-spin" /> : <Video size={28} />}
                 </button>
                 ...
                 >
                     {isGeneratingAudio ? <div className="w-5 h-5 border-2 border-black border-t-transparent rounded-full animate-spin"/> : isPlayingAudio ? <StopCircle size={28} /> : <Headphones size={28} />}
                 </button>
             </div>
        )}
    </div>
