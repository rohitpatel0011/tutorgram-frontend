...
interface Props {
  ...
  title?: string;
  xpReward?: number;
  passingScore?: number; // Raw number, e.g., 8 for 8/10
  onPass?: () => void;
}
...
const QuizModal: React.FC<Props> = ({
    ...
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 backdrop-blur-sm p-4">
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-2xl shadow-[8px_8px_0px_0px_#ABFA00] border-4 border-black text-center max-w-sm w-full">
          ...
          <h3 className="font-black text-xl mb-2 text-black dark:text-white">
            GENERATING EXAM
          </h3>
          <p className="text-gray-600 dark:text-gray-400 font-bold mb-6">
            Analyzing full module content...
          </p>
          <button
            onClick={onClose}
            className="text-sm font-black text-gray-400 hover:text-black dark:hover:text-white underline decoration-2 underline-offset-2 uppercase tracking-wide transition-colors">
            Cancel & Close
          </button>
        </div>
      </div>
  ...
  const handleNext = () => {
    if (isLastQuestion) {
      handleResult(
        score + (selectedOption === question.correctAnswerIndex ? 0 : 0),
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
      <div className="bg-white dark:bg-black rounded-2xl shadow-[10px_10px_0px_0px_#ABFA00] border-4 border-black dark:border-zinc-600 w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col relative">
        ...
        <div className="p-6 border-b-4 border-black dark:border-zinc-700 flex justify-between items-center bg-gray-50 dark:bg-zinc-900">
          ...
          <button
            onClick={resetQuiz}
            className="text-black dark:text-white hover:bg-gray-200 dark:hover:bg-zinc-800 p-2 rounded-lg transition-colors">
            <X size={24} />
          </button>
        </div>
            ...
            <div className="text-center py-8">
              <div className="mb-6 inline-block p-6 rounded-full bg-white dark:bg-zinc-800 border-4 border-black dark:border-zinc-500 shadow-[6px_6px_0px_0px_#000]">
                {score >= (passingScore || 8) ? (
                  <Trophy size={64} className="text-yellow-500 fill-current" />
                ) : (
              ...
              <div className="text-2xl font-bold mb-6 text-gray-700 dark:text-gray-300">
                You scored{" "}
                <span className="text-acid-dark dark:text-acid font-black text-4xl">
                  {score}
                </span>{" "}
                / {quizData.questions.length}
              </div>
              ...
              {score >= (passingScore || 8) ? (
                ...
                <p className="text-gray-500 mb-8 font-bold">
                  You need {passingScore || 8} correct answers to pass and earn
                  XP.
                </p>
              )}

              <button onClick={resetQuiz} className="neo-btn w-full md:w-auto">
                {score >= (passingScore || 8)
                  ? "CLAIM REWARD & CLOSE"
                  : "CLOSE & RETRY"}
              </button>
            ...
            <>
              ...
              <div className="space-y-4">
                {question.options.map((option, idx) => {
                  let optionClass =
                    "w-full text-left p-4 rounded-xl border-2 transition-all flex items-center justify-between group font-bold ";

                  if (isSubmitted) {
                    if (idx === question.correctAnswerIndex) {
                      optionClass +=
                        "border-green-600 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300";
                    } else if (idx === selectedOption) {
                      optionClass +=
                        "border-red-600 bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300";
                    } else {
                      optionClass +=
                        "border-gray-200 dark:border-zinc-700 text-gray-400 opacity-50";
                    }
                  } else {
                    if (selectedOption === idx) {
                      optionClass +=
                        "border-black bg-black text-white dark:bg-white dark:text-black shadow-[4px_4px_0px_0px_#acid]";
                    } else {
                      optionClass +=
                        "border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 hover:border-black dark:hover:border-white hover:bg-gray-50 dark:hover:bg-zinc-800 text-black dark:text-white";
                    }
                  }
                  ...
                  return (
                    <button
                      ...
                      <div className="flex items-center gap-4">
                        <span
                          className={`w-8 h-8 rounded-lg border-2 border-current flex items-center justify-center text-sm font-black`}>
              ...
              {isSubmitted && (
                <div className="mt-8 p-6 bg-blue-50 dark:bg-zinc-800 border-l-8 border-blue-500 rounded-r-xl">
                  <strong className="block text-xs font-black uppercase tracking-widest text-blue-600 mb-2">
                    Explanation
                  </strong>
