...
import { Brain, CheckCircle, XCircle, AlertCircle, RefreshCw, Trophy, ArrowRight } from 'lucide-react';
...
const TopicQuiz: React.FC<Props> = ({ topicTitle, topicContent, topicId, previousScore, onPass }) => {
  ...
  const [minimized, setMinimized] = useState(previousScore !== undefined && previousScore >= 4);
  ...
  const handleGenerate = async () => {
    ...
    const data = await generateTopicQuiz(topicTitle, topicContent);
    setQuizData(data);
    setLoading(false);
  };
  ...
  const handleSubmitAnswer = () => {
    if (!quizData) return;
    setIsSubmitted(true);
    if (selectedOption === quizData.questions[currentQIndex].correctAnswerIndex) {
      ...
      return (
        <div className="mt-12 border-4 border-green-500 bg-green-50 dark:bg-green-900/10 rounded-xl p-6 flex items-center justify-between shadow-[6px_6px_0px_0px_#166534]">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h3 className="font-black text-xl text-green-800 dark:text-green-400 uppercase tracking-tight">Topic Mastered!</h3>
                    <p className="font-bold text-green-700 dark:text-green-500">You scored {previousScore}/5. Great job!</p>
                </div>
            </div>
            <button 
    ...
    <div className="mt-16 bg-white dark:bg-black rounded-xl border-4 border-black dark:border-zinc-700 overflow-hidden shadow-[8px_8px_0px_0px_#000] dark:shadow-[8px_8px_0px_0px_#444]">
        ...
        <div className="bg-gray-50 dark:bg-zinc-900 p-6 border-b-4 border-black dark:border-zinc-700 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                ...
                <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Prove your knowledge on "{topicTitle}"</p>
            </div>
            {!quizData && !loading && (
                 <button 
                    onClick={handleGenerate}
        ...
        <div className="p-8 md:p-10">
            {loading && (
                <div className="text-center py-10">
                    ...
                    <p className="font-bold text-black dark:text-white text-lg">Brewing fresh questions...</p>
                </div>
            )}
            ...
            {quizData && !showResult && (
                <div className="max-w-3xl mx-auto">
                    ...
                    <div className="flex items-center gap-4 mb-10">
                        <span className="font-black text-black dark:text-white">Q{currentQIndex + 1}/5</span>
                        <div className="flex-1 h-4 bg-gray-200 dark:bg-zinc-800 rounded-full border-2 border-black dark:border-zinc-600 overflow-hidden">
                            <div className="h-full bg-acid border-r-2 border-black" style={{ width: `${((currentQIndex + 1) / 5) * 100}%` }}></div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 mb-10">
                        {quizData.questions[currentQIndex].options.map((opt, idx) => {
                            let statusClass = "border-black dark:border-zinc-600 hover:bg-gray-50 dark:hover:bg-zinc-800 hover:shadow-[4px_4px_0px_0px_#000] dark:hover:shadow-[4px_4px_0px_0px_#fff]";
                            let icon = null;

                            if (isSubmitted) {
                                if (idx === quizData.questions[currentQIndex].correctAnswerIndex) {
                                    statusClass = "bg-green-100 dark:bg-green-900 border-green-600 dark:border-green-400 text-green-900 dark:text-green-100";
                                    icon = <CheckCircle size={20} />;
                                } else if (idx === selectedOption) {
                                    statusClass = "bg-red-100 dark:bg-red-900 border-red-600 dark:border-red-400 text-red-900 dark:text-red-100";
                                    icon = <XCircle size={20} />;
                                } else {
                                    statusClass = "border-gray-200 dark:border-zinc-800 opacity-50";
                                }
                            } else if (selectedOption === idx) {
                                statusClass = "bg-acid border-black shadow-[4px_4px_0px_0px_#000] text-black font-bold";
                            }
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <span className={`w-8 h-8 rounded-md border-2 border-black dark:border-zinc-400 flex items-center justify-center text-sm font-black ${selectedOption === idx ? 'bg-black text-white' : 'bg-white dark:bg-black text-black dark:text-white'}`}>
                    ...
                    <div className="flex justify-end">
                        {!isSubmitted ? (
                            ...
                            >
                                {currentQIndex < 4 ? 'Next Question' : 'See Results'} <ArrowRight size={20} />
                            </button>
                        )}
                    </div>
                    ...
                    {isSubmitted && (
                        <div className="mt-6 p-5 bg-blue-50 dark:bg-zinc-900 border-l-4 border-blue-500 rounded-r-lg text-blue-900 dark:text-blue-100">
                            <strong className="block uppercase text-xs font-black tracking-widest mb-1 text-blue-500">Explanation</strong>
            ...
            {showResult && (
                <div className="text-center py-10">
                    <div className="mb-6 inline-block p-6 rounded-full bg-gray-100 dark:bg-zinc-800 border-4 border-black dark:border-zinc-600">
                        {score >= 4 ? <Trophy size={64} className="text-yellow-500" /> : <RefreshCw size={64} className="text-gray-400" />}
                    </div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mb-8 font-bold text-lg">
                        You got <span className="text-acid-dark dark:text-acid font-black text-2xl mx-1">{score}</span> / 5 correct.
                    </p>
